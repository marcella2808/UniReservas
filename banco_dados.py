...
class BancoDeDados:
    ...
    def criar_tabela(self):  # cria uma tabela para poder inserir os dados (usuário e senha)
        ...
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS usuarios ( 
                                ...
                                password TEXT NOT NULL)''')  # executa o comando SQL de criar tabela
    ...
    def validar_login(self, username, password):
        self.conectar()
        self.cursor.execute('''SELECT * FROM usuarios WHERE username = ? AND password = ?''', (username, password))  # realiza uma consulta
